<template>
  <div>
    ...
    <div v-for="rootRole in rootRoleWrapperListRef" :key="rootRole.root.code">
      <el-row justify="start">
        <el-col :span="20">
          <el-text type="primary" tag="b" size="large" style="float: left">{{" > " + rootRole.root.parentNameList.join(" > ")}}</el-text>
        </el-col>
      </el-row>

      <el-row justify="start">
        <el-col>
          <el-tree ref="elTreeRef" :data="[rootRole.root]" :props="defaultProps" show-checkbox check-strictly node-key="code"
                   :default-checked-keys="rootRole.selectedCodeList" :default-expanded-keys="rootRole.selectedCodeList"
                   @check="onCheck"/>
        </el-col>
      </el-row>
...
import {h, ref} from "vue";
  ...
  // 可选择的根角色数据列表
  const rootRoleWrapperListRef = ref([])
  // 目标用户原来已存在的角色列表
  ...
  // 查询权限树数据
  HttpApi.get(`/api/auth/admin/user/authority/listAvailableTree?username=${selectedRowRef.value.username}`,
      response => {
        rootRoleWrapperListRef.value = response.data.map(item => {
          let selectedCodeList = findGrantedAuthorityCodes(item, [])

          // 记录目标用户原来已存在的角色集合
          originalCheckedRoleList.push(...selectedCodeList)
  ...
  /**
   * 从指定根角色开始，递归查找用户已拥有的权限编码
   * @param root
   * @param grantedCodes
   * @returns
   */
  function findGrantedAuthorityCodes(root, grantedCodes){
    if(root.granted){
      grantedCodes.push(root.code)
    }

    if(root.children.length > 0){
      root.children.forEach(child => findGrantedAuthorityCodes(child, grantedCodes))
    }
  ...
  function save(){
    ElMessageBox({
      ...
      HttpApi.post("/api/auth/admin/user/authority/grant", data, config,
          () => {
