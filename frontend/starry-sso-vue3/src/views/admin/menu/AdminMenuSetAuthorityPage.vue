<template>
  <div>
    <el-row justify="center">
      <el-col :span="12"><el-text type="primary" size="large" tag="b">菜单项编码：{{selectedRowRef.code}}</el-text></el-col>
      <el-col :span="12"><el-text type="primary" size="large" tag="b">菜单项名称：{{selectedRowRef.name}}</el-text></el-col>
    </el-row>
...
// 查询权限树数据
HttpApi.post(`/api/auth/admin/menu/authority/listTree?menuItemCode=${selectedRowRef.value.code}`, null, null,
    response => {
...
function save(){
  ElMessageBox({
    ...
    const data = {
      menuItemCode: selectedRowRef.value.code,
      newRoleCodeList: newRoleList,
      removeRoleCodeList: removeRoleList
    }
    HttpApi.post("/api/auth/admin/menu/authority/grant", data, config,
        () => {
