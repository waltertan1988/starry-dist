<template>
  <AdminLayout>
    <div style="text-align: left">
      ...
      <el-row justify="start">
        <el-col :span="7">
          <el-text tag="b">功能行名称：</el-text>
          <el-input v-model="nameRef" style="width: 200px;" placeholder="请输入功能行名称" clearable/>
        </el-col>

        <el-col :span="7">
          <el-text tag="b">功能行编码：</el-text>
          <el-input v-model="codeRef" style="width: 200px;" placeholder="请输入功能行编码" clearable/>
        </el-col>

        <el-col :span="4">
          <el-text tag="b">类型：</el-text>
          <el-select v-model="rowTypeRef" style="width: 100px;" placeholder="请选择">
            <el-option v-for="item in [{value: '', label: '全部'}, {value: 0, label: '功能项'}, {value: 1, label: '功能组'}]"
                       :key="item.value" :label="item.label" :value="item.value"/>
          </el-select>
        </el-col>

        <el-col :span="4" :push="1">
          <el-button type="primary" :icon="Search" @click="searchFunctionList">查询</el-button>
        </el-col>
      </el-row>
    </div>
    ...
    <el-row justify="start">
      <el-col :span="3">
        <el-button type="success" :icon="FolderAdd" :disabled="selectedRowRef == null || selectedRowRef.rowType === 0" plain bg @click="openOpDialog('createGroup')">添加功能组</el-button>
      </el-col>
      <el-col :span="3">
        <el-button type="primary" :icon="Plus" :disabled="selectedRowRef == null || selectedRowRef.rowType === 0" plain bg @click="openOpDialog('createItem')">添加功能项</el-button>
      </el-col>
      <el-col :span="3">
    ...
    <div>
      <el-table ref="tableRef" :data="functionDataListRef" :row-key="getTableRowKey" :tree-props="{children: 'resourceGroupVoList'}"
        ...
        <el-table-column prop="code" label="功能行编码" width="250" fixed/>
        <el-table-column prop="seq" label="顺序" width="65"/>
        <el-table-column prop="rowType" label="请求方法" :formatter="(row, column, cellValue) => cellValue === 1 ? null : row.httpMethodList.join(', ')" width="200"/>
        <el-table-column prop="rowType" label="路径模式" :formatter="(row, column, cellValue) => cellValue === 1 ? null : row.pattern" width="250"/>
  ...
  <el-dialog v-model="opDialogAttrRef.visible" :title="opDialogAttrRef.title" :width="opDialogAttrRef.width"
    ...
    <component :is="opDialogAttrRef.component" :selectedRow="opRowDataRef" :componentParam="opDialogAttrRef.componentParam"
               @submitSuccess="opDialogAttrRef.visible = false; searchFunctionList();"></component>
  </el-dialog>
</template>
...
import AdminFunctionGroupSavePage from "@/views/admin/function/AdminFunctionGroupSavePage.vue";
import AdminFunctionItemSavePage from "@/views/admin/function/AdminFunctionItemSavePage.vue";
import AdminFunctionSetAuthorityPage from "@/views/admin/function/AdminFunctionSetAuthorityPage.vue";
...
const tableRef = ref()
// 功能列表数据
const functionDataListRef = ref([])
// 数据是否加载中
const functionDataLoadingRef = ref(false)
...
function searchFunctionList(){
  const data = {
    rootResourceGroupCode: process.env.VUE_APP_ROOT_FUNCTION_GROUP,
    code: codeRef.value,
    name: nameRef.value,
  ...
  HttpApi.post('/api/auth/admin/function/list', data, config,
      response => {
        functionDataListRef.value = response.data
      },
      response => {
      ...
      () => {
        functionDataLoadingRef.value = true
      },
      () => {
        functionDataLoadingRef.value = false
      })
}
...
function openOpDialog(opType, row) {
  if(opType === 'createGroup'){
    ...
    opDialogAttrRef.value.title = "添加功能组"
    opDialogAttrRef.value.width = "50%"
    opDialogAttrRef.value.component = shallowRef(AdminFunctionGroupSavePage)

  }else if(opType === 'updateGroup'){
    opRowDataRef.value = row
    opDialogAttrRef.value.title = "编辑功能组"
    opDialogAttrRef.value.width = "50%"
    opDialogAttrRef.value.component = shallowRef(AdminFunctionGroupSavePage)

  }else if(opType === 'createItem'){
    opRowDataRef.value = selectedRowRef.value
    opDialogAttrRef.value.title = "添加功能项"
    opDialogAttrRef.value.width = "50%"
    opDialogAttrRef.value.component = shallowRef(AdminFunctionItemSavePage)

  }else if(opType === 'updateItem'){
    opRowDataRef.value = row
    opDialogAttrRef.value.title = "编辑功能项"
    opDialogAttrRef.value.width = "50%"
    opDialogAttrRef.value.component = shallowRef(AdminFunctionItemSavePage)
    ...
    opDialogAttrRef.value.width = "50%"
    opDialogAttrRef.value.component = shallowRef(AdminFunctionSetAuthorityPage)
  }
...
function moveSubTree(row){
  ...
  ElMessageBox.confirm(
      `你确定要把【${row.name}：${row.code}】${row.rowType === 1 ? '及其下所有功能行' : ''}，移动到【${selectedRow.name}：${selectedRow.code}】下面吗？`,
      '提示',
    ...
    HttpApi.post("/api/auth/admin/function/move", data, config,
        () => {
...
function deleteSubTree(row){
  ElMessageBox.confirm(
      `你确定要删除【${row.name}：${row.code}】${row.rowType === 1 ? '及其下所有功能行' : ''}吗？`,
      '提示',
      {confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning'}
  ).then(() => {
    const config = {
    ...
    HttpApi.post(`/api/auth/admin/function/delete?rowType=${row.rowType}&code=${row.code}`, null, config,
        () => {
