...
public class ChineseTokenTextSplitter extends TextSplitter {
    ...
    protected List<String> doSplit(String text, int chunkSize) {
        ...
        while (!tokens.isEmpty() && num_chunks < this.maxNumChunks) {
            ...
            int lastPunctuation = this.getLastPunctuationMark().stream()
                    .map(chunkText::lastIndexOf).max(Integer::compareTo).orElse(-1);
    ...
    protected Collection<Character> getLastPunctuationMark() {
        return Set.of('.', '。', '?', '？', '!', '！', '\n');
    }
